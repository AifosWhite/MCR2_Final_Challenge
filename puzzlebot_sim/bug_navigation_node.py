...
class BugNavigationNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f'Node initialized: bug_mode={self.bug_mode}, direction={self.bug_direction}'
        )
    ...
    def can_leave_wall(self, dist_to_goal, line_distance, ang_to_goal):
        ...
        if self.bug_mode == 2:
            return (
                line_distance <= self.line_distance_threshold_bug2
    ...
    def is_sector_clear(self, center_angle, half_width):
        ...
        for current_range in self.lidar.ranges:
            if np.isfinite(current_range):
                delta = np.arctan2(
                    np.sin(angle - center_angle),
    ...
    def goal_cb(self, msg):
        ...
        self.min_line_distance = np.inf
        self.get_logger().info(
            f'New goal received: ({self.goal_x_current:.2f}, {self.goal_y_current:.2f})'
        )
