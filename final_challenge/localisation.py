...
class Localisation(Node):
    def __init__(self):
        ...
        self.declare_parameter('aruco_soft_gate',        True)
        self.declare_parameter('aruco_gate_dist',        1.20)
        self.declare_parameter('aruco_gate_bearing',     2.20)
        self.declare_parameter('aruco_clip_dist',        0.45)
        self.declare_parameter('aruco_clip_bearing',     0.70)
        ...
        self.aruco_soft_gate    = bool(gp('aruco_soft_gate').value)
        self.aruco_gate_dist    = float(gp('aruco_gate_dist').value)
        self.aruco_gate_bearing = float(gp('aruco_gate_bearing').value)
        self.aruco_clip_dist    = float(gp('aruco_clip_dist').value)
        self.aruco_clip_bearing = float(gp('aruco_clip_bearing').value)
    ...
    def ekf_correct(self, marker_id, z_dist, z_bearing):
        ...
        raw_innovation = innovation.copy()

        # Gate robusto:
        # Antes se rechazaba con |dist| > 0.8 m o |bearing| > 0.8 rad.
        # En físico eso es demasiado agresivo: si la odometría/yaw ya viene mal,
        # justo el ArUco que debería corregir la pose queda rechazado.
        # Ahora solo se rechazan outliers extremos y, para errores medianos, se
        # recorta la innovación para corregir gradualmente sin saltos bruscos.
        if (abs(raw_innovation[0]) > self.aruco_gate_dist or
                abs(raw_innovation[1]) > self.aruco_gate_bearing):
            self.get_logger().warn(
                f'ArUco {marker_id} rechazado fuerte: '
                f'dist={raw_innovation[0]:+.3f} bearing={raw_innovation[1]:+.3f}',
                throttle_duration_sec=0.5)
            return

        if self.aruco_soft_gate:
            innovation[0] = float(np.clip(innovation[0],
                                          -self.aruco_clip_dist,
                                           self.aruco_clip_dist))
            innovation[1] = float(np.clip(innovation[1],
                                          -self.aruco_clip_bearing,
                                           self.aruco_clip_bearing))
            if (abs(raw_innovation[0] - innovation[0]) > 1e-6 or
                    abs(raw_innovation[1] - innovation[1]) > 1e-6):
                self.get_logger().warn(
                    f'ArUco {marker_id} corregido con clip: '
                    f'raw=({raw_innovation[0]:+.3f}, {raw_innovation[1]:+.3f}) '
                    f'clip=({innovation[0]:+.3f}, {innovation[1]:+.3f})',
                    throttle_duration_sec=0.5)
        ...
        self.get_logger().info(
            ...
            f'err={raw_innovation[0]:+.3f} bear_err={raw_innovation[1]:+.3f} | '
