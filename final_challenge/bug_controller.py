...
class BugController(Node):
    def __init__(self):
        ...
        self.declare_parameter('initial_controller_mode', 'p2p_controller')
        self.declare_parameter('initial_wall_side', 'right')
        ...
        self.initial_controller_mode = str(gp('initial_controller_mode').value)
        self.initial_wall_side = str(gp('initial_wall_side').value)
        ...
        if self.use_waypoint_list:
            ...
            if self.initial_controller_mode == 'following_walls':
                self.controller_mode = 'following_walls'
                if self.initial_wall_side in self.fw_dirs:
                    self.fw_dir = self.initial_wall_side
    ...
    def lidar_callback(self, msg):
        ...
        if self.controller_mode == 'p2p_controller':
            ...
            if self.target_line is None:
                self._set_target_line_from_current_goal()
            if self.controller_type == 'BUG0':
                ...
                followed_enough = self.start_pose is None or \
                    euclidean(self.robot_pose, self.start_pose) > \
                    self.min_wall_follow_distance
                ...
                if followed_enough and abs(err_y) < self.bug2_line_tol and \
    ...
    def _set_target_line_from_current_goal(self):
        m = (self.robot_setpoint.y - self.robot_pose.y) / \
            ((self.robot_setpoint.x - self.robot_pose.x) + 1e-6)
        b = self.robot_pose.y - m * self.robot_pose.x
        self.target_line = lambda x: m * x + b
